...
> 인터파크 티켓(NOL 티켓) 스타일의 예매 서비스를 뼈대로 삼아, 백엔드 성능 엔지니어링 학습용 프로젝트.
...
이 프로젝트의 본체는 "티켓 예매 서비스"가 아니라 **성능 엔지니어링 학습**이다. 콘서트/스포츠 티켓팅이라는 도메인은 아래와 같은 이유로 채택했다.

- 오픈런 시점의 트래픽 폭주 → 동시성 제어와 부하테스트를 자연스럽게 정당화
- 좌석 선점 → 결제 → 확정이라는 흐름 → 트랜잭션 경계 설계 연습에 적합
- 목록/상세 조회가 많음 → 실행계획 분석, 인덱스, 캐싱을 붙일 여지가 충분
