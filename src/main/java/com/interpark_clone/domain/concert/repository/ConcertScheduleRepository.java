...
public interface ConcertScheduleRepository extends JpaRepository<ConcertSchedule, Long> {
    ...
    @Query("""
            ...
            order by
              case when :sort = 'OPEN_AT' then cs.openAt end asc,
              case when :sort = 'LATEST' then cs.concert.createdAt end desc,
              case when :sort = 'VIEW' then cs.concert.viewCount end desc,
              cs.createdAt desc
            """)
