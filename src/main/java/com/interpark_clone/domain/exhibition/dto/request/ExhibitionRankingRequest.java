...
@ParameterObject
public record ExhibitionRankingRequest(
        @Schema(description = "랭킹 집계 기준일입니다. 생략하면 오늘 날짜로 조회합니다.", type = "string", format = "date", example = "2026-08-01")
        @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
        LocalDate rankingDate,
        @Parameter(
                description = "전시/행사 장르 필터입니다. 장르는 랭킹 집계 범위를 제한하는 필터이며, 순위는 확정 예매 매수 기준입니다.",
                schema = @Schema(type = "string", allowableValues = {"EXHIBITION", "MUSEUM", "EVENT_FESTIVAL", "CHILD_EXPERIENCE"})
        )
    ...
    public ExhibitionRankingRequest {
